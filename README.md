...
This directory contains the necessary files to deploy a RAG pipeline using the R2R framework with the SciPhi cloud platform.
...
The application is built using the `r2r` library and follows a pipeline-based architecture. The main entry point is the `app.py` file, which creates a pipeline using the `E2EPipelineFactory.create_pipeline()` method. This method reads the configuration settings from the `config.json` file to set up the various components of the pipeline.

The `config.json` file contains the following configuration options:

- `database`: Specifies the database provider. In this case, it is set to "sciphi".
- `evals`: Specifies the evaluation provider (set to "deepeval") and the evaluation frequency.
